...
class {{.Name}} extends \Module\Base
{
    ...
    public function create{{.Name}}(array $params)
    {
        ...
        //构建{{.Name}}数据
        ${{.NameLow}} = [{{range .Fd}}
            '{{.FieldName}}' => $params['{{.FieldName}}'],{{end}}
        ];
        //写入
...
{{if .Per.List}}
    ...
    public function get{{.Name}}List(array $searchParams, int $pageNo, int $pageSize)
    {
        ...
        {{range .Search}}
        // {{.SDes}}
        if (isset($searchParams['{{.SName}}']) && !empty($searchParams['{{.SName}}'])) {
            $cond['{{.SName}}'] = $searchParams['{{.SName}}'];
        }{{end}}
...
{{if .Per.Export}}
    ...
    /**
        *{{.Des}}.
     */
    public function {{.Fname}}({{if .NeedPk}} ${{.PK}},{{end}}{{if .ParamIsArr}}$params {{else}} {{range .Param}} ${{.}} {{end}} {{end}})
    {
        {{if .ParamIsArr}}
        {{range .Param}}
