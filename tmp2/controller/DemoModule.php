...
class {{.Name}} extends ModuleBase
{
    ...
    {
        return \Service\Operation\{{.Name}}::instance()->get{{.Name}}Detail(${{.NameLow}}Id);
    }
{{end}}
    {{if .Per.List}}
    /**
...
{{end}}
   {{if .Per.Delete}}
    ...
    {
        return \Service\Operation\{{.Name}}::instance()->delete{{.Name}}($opUid, ${{.NameLow}}Id);
    }
{{end}}
    {{if .Per.Edit}}
    /**
    ...
    {
        return \Service\Operation\{{.Name}}::instance()->edit{{.Name}}($opUid, ${{.NameLow}}Id, $params);
    }
{{end}}
    {{if .Per.Online}}
    /**
    ...
    {
        return \Service\Operation\{{.Name}}::instance()->doOnline($opUid, ${{.NameLow}}Id);
    }
{{end}}
    {{if .Per.Offline}}
    /**
    ...
    {
        return \Service\Operation\{{.Name}}::instance()->doOffline($opUid, ${{.NameLow}}Id);
    }
{{end}}
    {{if .Per.Export}}
    /**
    ...
    {
        return \Service\Operation\{{.Name}}::instance()->export{{.Name}}List($searchParams);
    }
    {{end}}
    ...
    {
        return \Service\Operation\{{.Name}}::instance()->audit($opUid, ${{.NameLow}}Id, $params);
    }
    {{end}}
     ...
     */
     ...
     {
        return \Service\Operation\{{$.Name}}::instance()->{{.Fname}}($opUid{{if .NeedPk}} , ${{.PK}}{{end}}{{if .ParamIsArr}}, $params {{else}} {{range .Param}} ,${{.}} {{end}} {{end}});
     }
    {{end}}
}
